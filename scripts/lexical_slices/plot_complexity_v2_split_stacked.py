"""Stacked line plot of MRR vs v2 complexity score (1..4) for three models.
...
OUT_PATH = REPO_ROOT / "plots" / "complexity_v2_split_mrr_stacked.pdf"
...
DATASETS = {
    "song_describer": ("songd_complexity_v2.json", "Song Describer"),
    "music_caps": ("mucaps_complexity_v2.json", "MusicCaps"),
}
SCORES = (1, 2, 3, 4)
SCORE_LABELS = {1: "1\n(tag-like)", 2: "2", 3: "3", 4: "4\n(non-tag)"}
...
def main() -> None:
    ...
    axes[-1].set_xlabel("Caption complexity (v2)", labelpad=2)
    axes[-1].set_xticks(list(SCORES))
    axes[-1].set_xticklabels([SCORE_LABELS[s] for s in SCORES])
