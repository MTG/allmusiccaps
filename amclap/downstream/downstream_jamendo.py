"""MTG-Jamendo autotagging downstream evaluation.
...
Supports three settings: autotagging_genre, autotagging_instrument, autotagging_moodtheme.
All three settings share the same embedding extraction (dataset_name = "mtg_jamendo").
"""
...
def _read_tsv(tsv_path: Path) -> list[dict]:
    """Read a Jamendo split TSV and return list of track dicts."""
    tracks = []
...
class JamendoEmbeddingDataset(Dataset):
    """Dataset for loading pre-extracted embeddings and multi-label Jamendo tags."""
    ...
    def __init__(
        ...
        self.class_names = sorted(tag_to_idx, key=lambda t: tag_to_idx[t])
        ...
        for track in tracks:
            ...
            emb_path = self.embeddings_dir / stem[:3] / f"{stem}.low.pt"
            ...
            try:
                ...
                warnings.warn(f"Could not load embedding for {track['path']}: {e}")
...
class JamendoDataModule(L.LightningDataModule):
    """DataModule for MTG-Jamendo autotagging with pre-extracted embeddings."""
    ...
    def __init__(
        ...
        # Read split files
        train_tracks = _read_tsv(train_tsv)
        val_tracks = _read_tsv(val_tsv)
        test_tracks = _read_tsv(test_tsv)

        print(
            f"Tracks - train: {len(train_tracks)}, "
            f"val: {len(val_tracks)}, test: {len(test_tracks)}"
        )

        # Build tag vocabulary from union of all splits
        all_tags: set[str] = set()
        for tracks in (train_tracks, val_tracks, test_tracks):
            for track in tracks:
    ...
    def _get_embedding_dim(self) -> int:
        for tracks in (self.train_tracks, self.val_tracks, self.test_tracks):
            for track in tracks:
                stem = Path(track["path"]).stem
                emb_path = self.embeddings_dir / stem[:3] / f"{stem}.low.pt"
                if emb_path.exists():
    ...
    def setup(self, stage=None):
        self.train_dataset = JamendoEmbeddingDataset(
            self.embeddings_dir, self.train_tracks, self.tag_to_idx
        )
        self.val_dataset = JamendoEmbeddingDataset(
            self.embeddings_dir, self.val_tracks, self.tag_to_idx
        )
        self.test_dataset = JamendoEmbeddingDataset(
            self.embeddings_dir, self.test_tracks, self.tag_to_idx
        )
