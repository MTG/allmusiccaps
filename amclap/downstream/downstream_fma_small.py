...
def load_tracks_metadata(metadata_file):
    """Load FMA tracks.csv with multi-level headers.

    Returns a dict mapping track_id (int) to genre_top (str).
    Only includes tracks from the 'small' subset.
    """
    tracks = pd.read_csv(metadata_file, index_col=0, header=[0, 1])

    # Filter to small subset
    small = tracks[tracks[("set", "subset")] == "small"]

    # Build track_id -> genre_top mapping
    genre_map = small[("track", "genre_top")].dropna().to_dict()
    return genre_map
...
def predict(
    ...
    # Load metadata to map track IDs to genres
    if metadata_file is None:
        raise ValueError("--metadata_file is required for FMA evaluation")
    genre_map = load_tracks_metadata(metadata_file)
    print(f"Loaded genre labels for {len(genre_map)} tracks from metadata")
    ...
    # Get embedding paths and labels from metadata
    embedding_paths = []
    y_true = []
    classes = set()
    skipped = 0
    for audio_path, *_ in eval_data_module.dataset.index.values():
        ...
        track_id = int(audio_name)

        if track_id not in genre_map:
            skipped += 1
            continue
        ...
        label = genre_map[track_id]
        classes.add(label)
        y_true.append(label)

    if skipped > 0:
        print(f"Skipped {skipped} tracks not found in metadata")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to FMA tracks.csv metadata file",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
